...
struct timer_test {
    ...
    future<> run() {
        ...
        t2.set_callback([this] { OK(); printf(" 900ms timer expired\n"); });
        t3.set_callback([this] { OK(); printf("1000ms timer expired\n"); });
        t4.set_callback([this] { OK(); printf("  BAD cancelled timer expired\n"); });
        t5.set_callback([this] { OK(); printf("1600ms rearmed timer expired\n"); pr1.set_value(); });
...
int main(int ac, char** av) {
    ...
    return app.run_deprecated(ac, av, [&t1, &t2] {
        printf("=== Start High res clock test\n");
        t1.run().then([&t2] {
            printf("=== Start Low  res clock test\n");
            return t2.run();
        }).then([] {
            printf("Done\n");
            engine().exit(0);
