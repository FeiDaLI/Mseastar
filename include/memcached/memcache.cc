...
private:
    ...
    void erase(item* item_ptr) {
        item& item_ref = *item_ptr; // 将 item* 转换为 item&
        if (IsInCache) {
    ...
    void expire() {
        ...
        while (!exp.empty()){
            auto item_ref = exp.begin(); // Directly get reference
            exp.pop_front();
...
public:
    ...
    std::pair<unsigned, foreign_ptr<std::shared_ptr<std::string>>> print_hash_stats() {
        ...
        return {engine().cpu_id(), make_foreign(std::make_shared<std::string>(ss.str()))};
    }
...
public:
    ...
    future<> print_hash_stats(output_stream<char>& out) {
        return _peers.map_reduce([&out] (std::pair<unsigned, foreign_ptr<std::shared_ptr<std::string>>> data) mutable {
    ...
    void start() {
        ...
        keep_doing([this] {
            return _chan.receive().then([this](udp_datagram dgram) {
                ...
                auto conn = std::make_shared<connection>(dgram.get_src(), request_id, std::move(in),
...
class tcp_server {
private:
    std::shared_ptr<server_socket> _listener;
    sharded_cache& _cache;
    distributed<system_stats>& _system_stats;
    uint16_t _port;
    struct connection {
...
public:
    ...
    void start() {
        ...
        _listener = std::make_shared<server_socket>(engine().listen(make_ipv4_address({_port}), lo));
        keep_doing([this] {
            return _listener->accept().then([this] (connected_socket fd, socket_address addr) mutable {
                auto conn = std::make_shared<connection>(std::move(fd), addr, _cache, _system_stats);
